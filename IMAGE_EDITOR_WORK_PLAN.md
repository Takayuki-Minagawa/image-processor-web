...
- [ ] P0-01 対象ユーザーと上位3ワークフローを確定する
- [ ] P0-02 最大画像寸法、レイヤー数、履歴数、対象端末を仮決定する
...
- [ ] P0-07 依存ライセンス、バンドルサイズ、CSP、更新状況を棚卸しする
- [ ] P0-08 ADR-001〜005を作成する
...
- [ ] P1-01 React / TypeScript / ViteのプロジェクトとCIを作る
- [ ] P1-02 メニュー、ツールバー、キャンバス、レイヤーパネルのアプリシェルを作る
- [ ] P1-03 `EditorDocument`、スキーマ検証、座標型を実装する
- [ ] P1-04 Command BusとUndo/Redoの最小実装を作る
- [ ] P1-05 Renderer Adapterを実装する
- [ ] P1-06 PNG / JPEG / WebPを開き、1レイヤーとして表示する
- [ ] P1-07 pan / zoom / fit / 100%表示を実装する
- [ ] P1-08 PNG書き出しをWorker経路で実装する
- [ ] P1-09 unit、component、Playwright E2Eの基盤を作る
- [ ] P1-10 エラー境界、処理中表示、キャンセルを実装する
...
- [ ] P2-01 レイヤー追加、複製、削除、名前、順序、表示、ロックを実装する
- [ ] P2-02 透明度と基本ブレンドモードを実装する
- [ ] P2-03 移動、拡大縮小、回転、反転、cropを実装する
- [ ] P2-04 ブラシ、消しゴム、スポイトを実装する
- [ ] P2-05 矩形、楕円、フリーハンド選択を実装する
- [ ] P2-06 選択の追加、削除、反転、コピー、切取、貼付を実装する
- [ ] P2-07 明るさ、コントラスト、彩度、色相、グレースケールを実装する
- [ ] P2-08 ぼかし、シャープを実装する
- [ ] P2-09 低解像度プレビューと確定処理を分ける
- [ ] P2-10 履歴パネルとショートカットを実装する
- [ ] P2-11 スライダー、ブラシストローク、変形を履歴トランザクション化する
- [ ] P2-12 数値入力による位置、サイズ、回転、透明度変更を実装する
...
- [ ] P3-01 バージョン付き独自プロジェクト形式を実装する
- [ ] P3-02 保存・再読込の画像golden testを作る
- [ ] P3-03 OPFS自動保存とCommandジャーナルを実装する
- [ ] P3-04 クラッシュ後の復旧候補一覧と復元を実装する
- [ ] P3-05 File System Access APIの直接保存をprogressive enhancementとして実装する
- [ ] P3-06 Blobダウンロードfallbackを実装する
- [ ] P3-07 PWAアプリシェルとオフライン起動を実装する
- [ ] P3-08 Service Worker更新時の未保存文書保護を実装する
- [ ] P3-09 ストレージ使用量、quota不足、データ消去の説明UIを作る
- [ ] P3-10 スキーママイグレーション試験を作る
...
- [ ] P4-05 CORS、SVG、ファイル名、CSP、依存関係をhardeningする
- [ ] P4-06 WCAG 2.2 AAに基づく自動検査と手動検査を行う
...
- [ ] P4-10 プライバシー説明、既知制限、対応形式を文書化する
...
1. 本文書のMVP対象・対象外をレビューする。
2. 対象ユーザー、最大画像、レイヤー数、最重要ワークフローを確定する。
3. GitリポジトリとReact / TypeScript / Viteの最小構成を初期化する。
4. Fabric.js / Konvaの比較PoC用fixtureと計測スクリプトを作る。
5. ADR-001〜005を作成し、Phase 1へ進む前に技術判断を確定する。
...
この計画は初期調査時点の仮説である。Phase 0で実画像・実ブラウザ・実端末の測定結果を取得し、採用技術、性能予算、期間を更新する。
