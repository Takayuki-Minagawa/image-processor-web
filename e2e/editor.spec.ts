...
async function createCanvas(
  ...
  await expect(page.getByText(`${width} × ${height}px`, { exact: true })).toBeVisible();
}
...
test.describe('Pixelweave editor', () => {
  ...
  test('初期表示が主要なアクセシブルnameを公開する', async ({ page }) => {
    ...
    await expect(
      page.getByRole('button', { name: 'やり直す' }),
    ).toBeDisabled();
    await expect(
      page.getByRole('button', { name: '書き出す' }),
    ).toBeVisible();
  ...
  test('連続したレイヤー追加を別々のUndo単位として扱う', async ({
    page,
  }) => {
    await createCanvas(page);
  ...
  test('書き出しダイアログで形式・品質・倍率を操作できる', async ({
    page,
  }) => {
    ...
    await expect(dialog.getByText('320 × 240 px', { exact: true })).toBeVisible();
    await expect(
    ...
    await expect(dialog.getByText('640 × 480 px', { exact: true })).toBeVisible();
    await expect(
